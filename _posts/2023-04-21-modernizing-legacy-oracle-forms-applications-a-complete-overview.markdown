...
- Even though Oracle APEX might be the easiest upgrade path from Oracle Forms, it's still not a trivial migration. You do not simply load the Forms source files into and magically generate a completed APEX application. It is a re-write, where a large part of the code base (PL/SQL in the database) may be re-used. The difficulty of the migration strongly depends on how customized and complex the individual Forms masks are and how big the Forms installation is in total.
- Does not introduce a clear separation between application- and database layer (which might be desirable for some people).
- Learning curve: Your team may need to learn new skills when transitioning from Oracle Forms to APEX.
...
## Modernizing with ORDS (Oracle RESTful Data Services)
...
## Modernizing with a custom implementation
